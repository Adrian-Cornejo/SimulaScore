   ...
   function obtenerExamenesEnProgresoPorProfesor($con, $codigoProfesor) {
    ...
    try {
        ...
        if (!empty($alumnos)) {
            ...
            $stmt = $con->prepare("SELECT * FROM resultados_examen_mejoredu WHERE en_progreso = 1 AND codigoAlumno IN ($inQuery)");
...
function mostrarExamenesEnProgresoPorProfesor($con, $codigoProfesor) {
    ...
    if (!empty($examenesEnProgreso)) {
        ...
        foreach ($examenesEnProgreso as $examen) {
            ...
            echo "<td>
                    <a href='verProgresoAlumno.php?codigoAlumno=" . urlencode($codigoAlumno) . "'>
                      Ver Progreso
                    </a>
                  </td>";
            echo "</tr>";
